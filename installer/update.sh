#!/usr/bin/env bash
# Update opanel from GitHub.
#
# By default this script downloads the newest stable release zip to a temporary
# directory, syncs the source into /opt/opanel, rebuilds the frontend, refreshes
...
if [[ -z "${opanel_UPDATE_STABLE_COPY:-}" ]]; then
  ...
  opanel_UPDATE_STABLE_COPY="$_stable_copy" \
    opanel_UPDATE_ORIGINAL_SCRIPT="$_original_script" \
...
# --- Config ----------------------------------------------------------------
APP_DIR="${APP_DIR:-/opt/opanel}"                 # Production deployment dir
DEFAULT_SOURCE_DIR="/opt/opanel-source"           # Dev/branch checkout dir only

# Resolve where THIS script lives. If it's inside a real git checkout we use
# that. Otherwise we fall back to /opt/opanel-source so users running the
# script from the deploy dir still get a usable workflow.
_SCRIPT_SOURCE="${opanel_UPDATE_ORIGINAL_SCRIPT:-${BASH_SOURCE[0]}}"
...
download_release_source() {
  ...
  RELEASE_WORK_DIR="$(mktemp -d /tmp/opanel-release-update.XXXXXX)"
  archive="${RELEASE_WORK_DIR}/opanel-release.zip"
...
PY
  if id -u opanel >/dev/null 2>&1; then
    chown opanel:opanel "$UPDATE_STATE_FILE" 2>/dev/null || true
  fi
...
configure_fastcgi_cache() {
  ...
  cat >/etc/nginx/conf.d/00-opanel-fastcgi-cache.conf <<'NGINX'
fastcgi_cache_path /var/cache/nginx/opanel-fastcgi levels=1:2 keys_zone=opanel_FASTCGI:32m inactive=30m max_size=256m use_temp_path=off;
...
harden_existing_panel_users() {
  ...
  while IFS= read -r user; do
    [[ -n "$user" ]] || continue
    case "$user" in
      root|daemon|bin|sys|sync|games|man|lp|mail|news|uucp|proxy|www-data|backup|list|irc|_apt|nobody|opanel|opanel-sites|opanel-sftp|mysql|redis|nginx)
        continue ;;
    esac
...
install_panel_runtime() {
  ...
  usermod -aG opanel-sites opanel 2>/dev/null || true
  usermod -aG opanel-sites www-data 2>/dev/null || true
  harden_existing_panel_users
  install -d -o root -g opanel -m 2775 /etc/nginx/conf.d
  chmod g+s /etc/nginx/conf.d 2>/dev/null || true
  install -d -o root -g opanel -m 2775 /etc/nginx/opanel/custom
  chmod g+s /etc/nginx/opanel/custom 2>/dev/null || true
  install -d -o opanel -g opanel -m 0750 /var/lib/opanel
  if command -v sshd >/dev/null 2>&1; then
    sshd_config="/etc/ssh/sshd_config"
    sshd_backup="${sshd_config}.opanel.bak"
...
SSHD
    if sshd -t; then
      ...
      echo "WARNING: invalid SSHD configuration; skipped opanel SFTP password block"
    fi
  fi
...
STARTER
  ...
  cat >/etc/systemd/system/opanel-api.service.d/20-panel-port.conf <<SERVICE
[Service]
...
ProtectHome=false
ReadWritePaths=
ReadWritePaths=${APP_DIR} /home /var/backups/opanel /etc/nginx/conf.d /etc/nginx/opanel/custom /tmp /var/lib/opanel
SERVICE
...
ProtectSystem=false
ProtectHome=false
ReadWritePaths=/home /var/backups/opanel /etc/nginx/conf.d /etc/nginx/opanel/custom /tmp /var/lib/opanel ${APP_DIR}
PrivateTmp=true
...
refresh_opanel_mariadb_grants() {
  ...
  "$mysql_bin" <<SQL
CREATE USER IF NOT EXISTS 'opanel'@'localhost' IDENTIFIED BY '${password}';
ALTER USER 'opanel'@'localhost' IDENTIFIED BY '${password}';
...
host=localhost
MYCNF
  chown opanel:opanel "$defaults_file"
  chmod 0600 "$defaults_file"
}

ensure_panel_runtime_ownership() {
  id -u opanel >/dev/null 2>&1 || return 0
  [[ -d "$APP_DIR/backend" ]] && chown -R opanel:opanel "$APP_DIR/backend" 2>/dev/null || true
  [[ -d "$APP_DIR/frontend" ]] && chown -R opanel:opanel "$APP_DIR/frontend" 2>/dev/null || true
...
backup_db() {
  local db_path="$APP_DIR/backend/opanel.db"
  if [[ ! -f "$db_path" ]]; then
    return 0
  fi
  local snap_dir="${BACKUP_ROOT:-/var/backups/opanel}/db-snapshots"
  install -d -m 0750 "$snap_dir"
  if id -u opanel >/dev/null 2>&1; then
    chown opanel:opanel "$snap_dir" 2>/dev/null || true
  fi
  local stamp
  stamp=$(date -u +%Y%m%d-%H%M%S)
  local snap="$snap_dir/opanel-$stamp.db"
...
if [[ -f "$SOURCE_DIR/installer/files/opanel-helper.sh" ]]; then
  log "Refreshing /usr/local/sbin/opanel-helper and /etc/sudoers.d/opanel"
...
if [[ -f "$SOURCE_DIR/change_IP.sh" ]]; then
  ...
  install -m 0755 -o root -g root "$SOURCE_DIR/change_IP.sh" /usr/local/sbin/opanel-change-ip
fi

if [[ -f "$SOURCE_DIR/installer/update.sh" ]]; then
  log "Refreshing panel update command"
  install -m 0755 -o root -g root "$SOURCE_DIR/installer/update.sh" /usr/local/sbin/opanel-update
fi

if [[ -f "$SOURCE_DIR/installer/files/opanelctl" ]]; then
  log "Refreshing SSH menu command: opanel"
  install -m 0755 -o root -g root "$SOURCE_DIR/installer/files/opanelctl" /usr/local/sbin/opanel
...
# --- Restore ownership so opanel user can read/write the deploy ------------
if id -u opanel >/dev/null 2>&1; then
  ensure_panel_runtime_ownership
fi
...
log "Restarting opanel-api"
mkdir -p /etc/systemd/system/opanel-api.service.d
cat >/etc/systemd/system/opanel-api.service.d/10-opanel-helper.conf <<'SERVICE'
[Service]
NoNewPrivileges=false
ProtectSystem=false
...
echo "  journalctl -u opanel-api -n 100 --no-pager"
