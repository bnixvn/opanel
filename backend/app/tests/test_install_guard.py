...
Running it a second time recreated the environment file and wiped opanel.db,
...
def test_the_guard_sits_before_any_destructive_step():
    ...
    guard = content.index('if [[ -f "${APP_DIR}/backend/opanel.db" ]]; then')

    # APP_DIR has to be resolved by then, or the guard checks the wrong path.
    assert content.index('APP_DIR="${APP_DIR:-/opt/opanel}"') < guard
    ...
    assert "opanel-update" in content[guard:guard + 1200]
...
def test_installer_stops_when_the_database_already_exists(tmp_path):
    app_dir = tmp_path / "opanel"
    (app_dir / "backend").mkdir(parents=True)
    (app_dir / "backend" / "opanel.db").write_bytes(b"SQLite format 3\x00")
