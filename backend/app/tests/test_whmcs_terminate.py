...
MODULE = PROJECT_ROOT / "modules" / "servers" / "opanel" / "opanel.php"
CONTRACT = PROJECT_ROOT / "docs" / "whmcs-opanel-contract.md"
HELPER = PROJECT_ROOT / "installer" / "files" / "opanel-helper.sh"


def _terminate_function() -> str:
    source = MODULE.read_text(encoding="utf-8")
    start = source.index("function opanel_TerminateAccount(")
    return source[start:source.index("\nfunction ", start + 1)]


def test_module_sends_no_backup_parameter():
    body = _terminate_function()
    call = next(line for line in body.splitlines() if "opanel_request(" in line)

    assert "'DELETE'" in call
    assert "backup" not in call
    # opanel_request($params, $method, $path, $payload = null, $query = []).
...
def test_contract_does_not_sell_backups_as_a_rollback_path():
    ...
    assert "/var/backups/opanel" in terminate
